...
// AccessToken is the Slack User Token (xoxp-...) obtained during "Sign in with Slack".
// It is used directly to send messages on behalf of the user — no Bot Token needed.
type User struct {
	...
	// AccessToken is the Slack User OAuth token (xoxp-...), stored encrypted at rest.
...
type UserSettings struct {
	...
	// CooldownMinutes is the interval before re-sending an auto-reply to the same sender.
	...
	// ReplyInThread controls whether auto-replies are sent as a thread reply.
	ReplyInThread    bool      `json:"reply_in_thread" db:"reply_in_thread"`
	// NotifyOnResume controls whether to notify senders when the user ends deep work.
